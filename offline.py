...
from libs.misc.saving_and_loading import save_cur_iter_dynamics_model, \
    save_cur_iter_behavior_policy, save_cur_iter_policy, \
    ...
    confirm_restoring_behavior_policy, confirm_restoring_value
...
def train(params):
    ...
    behavior_policy.running_stats.update_stats(behavior_policy_train_collection.data["observations"])
    ...
    if confirm_restoring_dynamics_model(params):
        ...
        save_cur_iter_dynamics_model(
                params, dynamics_saver, sess, itr=0
            )
    ...
    if confirm_restoring_behavior_policy(params):
        ...
        behavior_policy.fit_as_bc(behavior_policy_train_collection, behavior_policy_val_collection, behavior_policy_rollout_sampler)
        save_cur_iter_behavior_policy(params, behavior_policy_saver, sess, itr=0)

    # re-initialize TRPO policy with BC policy
    if params['bc_init']:
        logger.info("Initialize TRPO policy with BC.")
        update_weights = [tf.assign(new, old) for (new, old) in zip(tf.trainable_variables('policy'), tf.trainable_variables('behavior_policy'))]
        ...
        if rollout_sampler:
            ...
            logger.info("TRPO policy initialized with BC average return: " + str(returns))
